...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					teks = body.slice(7)
					reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
					buff = await getBuffer(`https://api.vhtear.com/write?text=${teks}&apikey=${RamlanID}`)
				...
				reply(ind.wait())
				buffer = await getBuffer(`https://api.xteam.xyz/textpro/ninjalogo?text=${nin}&text2=${ja}&APIKEY=${YOUR_APIKEY}`)
		...
		case 'halloweentext':
				...
				reply(ind.wait())
				buffer = await getBuffer(`https://api.xteam.xyz/textpro/helloweenfire?text=${ween}&APIKEY=${YOUR_APIKEY}`)
		    baby.sendMessage(from, buffer, image, {quoted: mek})
		    await limitAdd(sender)	
		    break
				case 'pornhub':
				...
				reply(ind.wait())
				buffer = await getBuffer(`https://api.xteam.xyz/textpro/ph?text=${porn}&text2=${hub}&APIKEY=${YOUR_APIKEY}`)
				baby.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
                case 'gemboktext':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					reply(ind.wait())
					buffer = await getBuffer(`https://api.vhtear.com/padlock?text1=${gem}&text2=${bok}&apikey=${RamlanID}`)
					baby.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
					break
                case 'glitchtext':
				...
				if (!isPrem) return reply(ind.premium())
					...
					reply(ind.wait())
					buffer = await getBuffer(`https://api.xteam.xyz/textpro/glitch?text=${gli}&text2=${tch}&APIKEY=${YOUR_APIKEY}`)
					baby.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
					break
				case 'simi':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					teks = body.slice(5)
					anu = await fetchJson(`https://api.xteam.xyz/simsimi?kata=${teks}&APIKEY=${YOUR_APIKEY}`)
					reply(anu.jawaban)
					break					
				case 'tts':
				if (isBanned) return reply(ind.baned())
				...
				if (args.length < 1) return baby.sendMessage(from, 'Kode bahasanya mana kak? contoh : ${prefix}tts id Halo Ramlan', text, {quoted: mek})
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/text2image?text=${teks}&apiKey=${IDxO1TFYnKADlX4pxcHa}`, {method: 'get'})
                ...
                var gh = body.slice(12)
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     teks = body.slice(9)
                     anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${teks}&apikey=${RamlanID}`, {method: 'get'})
		...
		case 'silktext':
				...
				reply(ind.wait())
				buffer = await getBuffer(`https://api.vhtear.com/silktext?text=${silk}&apikey=${RamlanID}`)
		    baby.sendMessage(from, buffer, image, {quoted: mek})
		    await limitAdd(sender)	
		    break
// PREMIUM
		...
		anu = await fetchJson(`https://api.vhtear.com/ytdl?link=${args[0]}&apikey=${RamlanID}`, {method: 'get'})
				...
				if (!isPrem) return reply(ind.premium())
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('URL NYA TIDAK VALID KAK')
					anu = await fetchJson(`https://api.vhtear.com/tiktokdl?link=${args[0]}&apikey=${RamlanID}`,)
...
neon = body.slice(10)
reply('「❗」WAIT GANS')
anu = await getBuffer(`https://api.xteam.xyz/textpro/neon?text=$APIKEY=${YOUR_APIKEY}`)
...
lol = await getBuffer(`https://api.xteam.xyz/textpro/blackpink?text=${pink}&APIKEY=${YOUR_APIKEY}`)
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname)) 
                reply(ind.wait())
                anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(6)}&apikey=${RamlanID}`)
               if (anu.error) return reply(anu.error)
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/randomnekonime&apikey=${RamlanID}`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					reply(ind.wait())
					anu = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${RamlanID}`, {method: 'get'})
					buffer = await getBuffer(anu.result.result)
					baby.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender)
					break										
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
						data = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${RamlanID}`)
...
buffer = await getBuffer(`https://api.xteam.xyz/textpro/cloudtext?text=${cloud}&APIKEY=${YOUR_APIKEY}`)
baby.sendMessage(from, buffer, image, {quoted: mek})
break
